...
import { FlowProvider, NonEVMMode, useNonEVMContext } from "../../providers/NonEVMContextProvider";
...
export default function FlowProviderModal({ visible, onCancel }: ProviderModalProps): JSX.Element {
  ...
  const { loadNonEVMModal, setFlowProviderConfig } = useNonEVMContext();
  const [lilicoInstalled, setLilicoInstalled] = useState(false);
  ...
  useEffect(() => {
    ...
    /* @ts-ignore */
    const lilicoExtension = window.fcl_extensions?.find(item => {
      return item.uid === "Lilico";
    });
    setLilicoInstalled(lilicoExtension !== undefined);
    // eslint-disable-next-line
          ...
          <Spin spinning={false}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr", rowGap: 8 }}>
              <NonEVMProvider
                ...
                onClick={() => {
                  setFlowProviderConfig(FlowProvider.blocto);
                  handleSelectProvider();
                  localStorage.setItem(storageConstants.KEY_FLOW_CONNECTED_WALLET_NAME, "flowBlockto");
                }}
              />
              <NonEVMProvider
                provider={{
                  logo: "./flowLilicoWallet.png",
                  name: lilicoInstalled ? "Lilico Wallet" : "Lilico Wallet (Install First)",
                }}
                onClick={() => {
                  if (lilicoInstalled) {
                    setFlowProviderConfig(FlowProvider.lilico);
                    handleSelectProvider();
                    localStorage.setItem(storageConstants.KEY_FLOW_CONNECTED_WALLET_NAME, "flowLilico");
                  } else {
                    window.open("https://chrome.google.com/webstore/detail/lilico/hpclkefagolihohboafpheddmmgdffjm");
                  }
                }}
              />
            </div>
