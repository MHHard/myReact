...
export enum PeggedChainMode {
  ...
  Burn,
  BurnThenSwap,
  TransitionPegV2, // used to peg v0 to v2 transition, if this value is not null, stand for it's a v2 peg mode, burn the v0 supply first.
}
  ...
  getSpenderAddress() {
    switch (this.mode) {
      ...
      case PeggedChainMode.TransitionPegV2:
        return this.config.migration_peg_burn_contract_addr; // default approve to the v0 peg contract
