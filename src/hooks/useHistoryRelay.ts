...
export const useHistoryRelay = (
  ...
  useEffect(() => {
    ...
    const depositConfigs = transferConfig?.pegged_pair_configs?.find(
      e => e.org_chain_id === fromChainId && e.pegged_chain_id === toChainId && e.org_token.token.symbol === symbol,
    );
    const burnConfigs = transferConfig?.pegged_pair_configs?.find(
      e => e.org_chain_id === toChainId && e.pegged_chain_id === fromChainId && e.org_token.token.symbol === symbol,
    );
  ...
  const speedUp = async (): Promise<string | undefined> => {
    ...
    if (!res.getErr()) {
      ...
      let relay;
      if (!peggedTokenBridge || !originalTokenVault || !dstBridge || !peggedTokenBridgeV2 || !originalTokenVaultV2)
        return undefined;
      try {
        switch (speedUpPeggedMode) {
          case PeggedChainMode.Deposit:
            if (!pegConfig) break;
            relay =
              pegConfig.bridge_version > 0
                ? await peggedTokenBridgeV2.mint(wdmsg, sigs, signers, powers)
                : await peggedTokenBridge.mint(wdmsg, sigs, signers, powers);
            break;
          case PeggedChainMode.Burn:
          case PeggedChainMode.BurnThenSwap:
            if (!pegConfig) break;
            relay =
              pegConfig.vault_version > 0
                ? await originalTokenVaultV2.withdraw(wdmsg, sigs, signers, powers)
                : await originalTokenVault.withdraw(wdmsg, sigs, signers, powers);
            break;
          default:
            if (multiBurnConfig) {
