...
import { aptosWallets, NonEVMMode, useNonEVMContext } from "../../providers/NonEVMContextProvider";
...
export default function AptosProviderModal({ visible, onCancel }: ProviderModalProps): JSX.Element {
  const { isMobile } = useAppSelector(state => state.windowWidth);
  const classes = useStyles({ isMobile });
  ...
  const handleSelectProvider = async walletName => {
    localStorage.setItem(storageConstants.KEY_APTOS_CONNECTED_WALLET_NAME, walletName);
    const mode: NonEVMMode =
      process.env.REACT_APP_ENV === "TEST" || process.env.REACT_APP_ENV === "DEV"
        ? NonEVMMode.aptosDevnet
        : NonEVMMode.aptosMainnet;
              ...
              {aptosWallets?.map(item => {
                return (
                  <Provider
                    ...
                    key={item?.name}
                    onClick={() => {
                      handleSelectProvider(item?.name);
                    }}
                  />
                );
              })}
