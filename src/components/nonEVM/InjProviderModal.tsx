...
import leapIcon from "../../images/leap.jpeg";
...
export default function InjProviderModal({ visible, onCancel }: ProviderModalProps): JSX.Element {
  const { isMobile } = useAppSelector(state => state.windowWidth);
  const classes = useStyles({ isMobile });
        ...
        <div style={{ width: "100%" }}>
          <Spin spinning={false}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr", rowGap: 8 }}>
              ...
              />
              <Provider
                provider={{ icon: leapIcon, name: "leap" }}
                onClick={() => {
                  handleSelectProvider("leap");
                }}
              />
            </div>
          </Spin>
