...
import type { MulticallState, WithMulticallState } from './types'
...
async function fetchChunk(
  ...
  chunk: Multicall2.CallStruct[],
  blockNumber: number,
  isDebug?: boolean
): Promise<Multicall2.ResultStructOutput[]> {
  try {
    const returnData  = await multicall.callStatic.tryAggregate(false,
      chunk.map((obj) => ({
        target: obj.target,
...
function onFetchChunkSuccess(
  ...
  chunk: Multicall2.CallStruct[],
  result: Multicall2.ResultStructOutput[]
) {
  ...
  const { erroredCalls, results } = chunk.reduce<{
    erroredCalls: Multicall2.CallStruct[]
...
function onFetchChunkFailure(context: FetchChunkContext, chunk: Multicall2.CallStruct[], error: any) {
