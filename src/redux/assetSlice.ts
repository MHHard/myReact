...
export const getTokenSymbol = (symbol, chId) => {
  ...
  if (chId === 1 || chId === 42161 || chId === 10 || chId === 5 || chId === 288 || chId === 42170) {
    if (symbol === "WETH") {
      name = "ETH";
    }
  }
...
export const getTxLink = (chainId: string | number, txLink: string) => {
  try {
    if (chainId === 999999998) {
      return `${getNetworkById(chainId).blockExplorerUrl}`;
    }
    return txLink;
  } catch (error) {
    console.error(error);
