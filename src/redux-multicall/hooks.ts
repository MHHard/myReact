...
import type { CallResult, CallState, ListenerOptionsWithGas, WithMulticallState } from './types'
...
export function useCallsDataSubscription(
  ...
  calls: Array<Multicall2.CallStruct | undefined>,
  blocksPerFetch = 1
): CallResult[] {
...
function useMultichainCallsDataSubscription(
  context: MulticallContext,
  chainToCalls: Record<number, Array<Multicall2.CallStruct | undefined>>,
  blocksPerFetch = 1
...
export function useSingleContractMultipleData(
  ...
  const calls = useMemo(() => {
    if (!contract) return []
    return callDatas.map<Multicall2.CallStruct | undefined>((callData) => {
...
export function useMultipleContractSingleData(
  ...
  const calls = useMemo(() => {
    if (!callData) return []
    return addresses.map<Multicall2.CallStruct | undefined>((target) => {
...
export function useSingleContractWithCallData(
  ...
  const calls = useMemo(() => {
    if (!contract) return []
    return callDatas.map<Multicall2.CallStruct>((callData) => ({
      target: contract.address,
      callData,
    }))
  }, [contract, callDatas, gasRequired])
...
export function useMultiChainMultiContractSingleData(
  ...
  const chainToCalls = useMemo(() => {
    ...
    return getChainIds(chainToAddresses).reduce((result, chainId) => {
      ...
      const calls = addresses.map<Multicall2.CallStruct | undefined>((target) => {
        if (!target) return undefined
        return { target, callData, gasRequired }
      })
      result[chainId] = calls
      return result
    }, {} as Record<number, Array<Multicall2.CallStruct | undefined>>)
