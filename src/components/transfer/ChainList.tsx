...
import { needToBlockAggrateOrAdd, sortedChainIds } from "../../constants/const";
...
const ChainList: FC<IProps> = ({ visible, onSelectChain, onCancel }) => {
  ...
  const { chainId, CHAIN_LIST } = useWeb3Context();
  ...
  const sortChainList = (chainList: Chain[]) => {
    ...
    chainList?.forEach(chain => {
      if (sortedChainIds.find(cId => chain.id === cId)) {
    ...
    sortedChainIds?.forEach(cId => {
      const targetNormalChain = normalChainList.find(chain => chain.id === cId);
        ...
        <div className={classes.litem}>
          <div className={classes.itemLeft}>
            ...
            <div style={{ marginLeft: 8 }}>
              <div id="chainName" className={classes.tokenName}>
                {chain.name}
              </div>
            </div>
          </div>
